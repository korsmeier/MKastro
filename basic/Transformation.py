...
def SolarModulationTn( Tn, F, phi, Z, A):
    Tn_new    = Tn - np.fabs(1.*Z)/A*phi;
    F_new           = F*(Tn_new*Tn_new+2*Tn_new*m_proton['GeV'])/(Tn*Tn+2*Tn*m_proton['GeV'])
    return Tn_new, F_new

def SolarModulationTn_interpolated( Tn_res, Tn, F, phi, Z, A):
    Tn_original = Tn_res + np.fabs(1.*Z)/A*phi
    F_intp      = np.exp( np.interp( np.log(Tn_original), np.log(Tn), np.log(F+1e-44) ) )
    F_new           = F_intp*(Tn_res**2+2*Tn_res*cst.m_proton['GeV'])/(Tn_original**2+2*Tn_original*cst.m_proton['GeV'])
    return  F_new
